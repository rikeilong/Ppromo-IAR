...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, attention=True, t_att=True):
    ...
    def forward(self, x):
        ...
        # if self.t_att:
        #     t_feats = x
        #     t_feats = t_feats.mean(1)
        #     N,T,_ = t_feats.size()

        #     t_feats_new = np.full((N,T),0,float)
        #     for n in range(N):
        #         for t in range(T):
        #             dists = []
        #             for j, v in enumerate([10, 9, 15, 16]):
        #                 dists.append((t_feats[n][t][v] - t_feats[n][t][2]).detach().numpy())
        #             t_feats_new[n,t] = np.abs(np.mean(dists))
        #     # t_weights = F.sigmoid((t_feats*t_feats).sum(dim=2)**0.5)
        #     t_weights = F.sigmoid(t_feats.mean(2))
...
if __name__ == "__main__":
    ...
    graph_args = {'layout': 'smarthome17','strategy': 'spatial'}
    model = Model(num_classes, 17, 1, 'smarthome.Graph',graph_args,2,attention=True)
    pred,t_extract,t_weights = model(input_tensor)
    # print(output.size())
